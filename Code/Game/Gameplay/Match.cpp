...
{
    if (ExecuteMove(fromCoords, toCoords, promoteTo, isTeleport)) g_eventSystem->FireEvent("OnExitMatchTurn");
}
                        ...
                        bool const     isTeleport)
{
    ...
    Piece* fromPiece = GetPieceByCoords(fromCoords);
    g_devConsole->AddLine(DevConsole::INFO_MAJOR, Stringf("Move Player #%d's %s from %s to %s", g_game->GetCurrentPlayerControllerId(), GetPieceByCoords(fromCoords)->m_definition->m_name.c_str(), m_board->ChessCoordToString(fromCoords).c_str(),
...
STATIC bool Match::OnChessMove(EventArgs& args)
{
    ...
    {
        g_devConsole->AddLine(DevConsole::ERROR, "ChessMove requires from= and to= parameters");
        return false;
    }
